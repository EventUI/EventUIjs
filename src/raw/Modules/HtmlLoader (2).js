...
/**Callback function that is called when a html placeholder has been completely loaded and injected into the DOM.
 @param {EVUI.Modules.HtmlLoader2.HtmlPlaceholderLoadResult} placeholderResult The details of the completed placeholder load.*/
EVUI.Modules.HtmlLoader2.Constants.Fn_GetPlaceholder_Callback = function (placeholderResult) { }

/**Callback function that is called when a html placeholder has been completely loaded and injected into the DOM.
 @param {EVUI.Modules.HtmlLoader2.HtmlPlaceholderLoadResult[]} placeholderResult The details of all the completed placeholder loads.*/
...
EVUI.Modules.HtmlLoader2.HtmlLoader2Controller = function (services)
{
    ...
    this.loadHtml = function (htmlRequestArgsOrUrl, getHtmlCallback)
    {
        ...
        EVUI.Modules.Http.Http.executeRequest(htmlArgs.httpArgs, function (completedRequest)
        {
            ...
            {
                EVUI.Modules.Core.Utils.debugReturn("EVUI.Modules.HtmlLoader2Controller.Manager", "GetHtml", completedRequest.error.getErrorMessage());
    ...
    this.loadAllPlacehoders = function (placehoderLoadArgs, callback)
    {
    ...
    this.loadAllPlacehodersAsync = function (placehoderLoadArgs)
    {
        return new Promise(function (resolve)
        {
    ...
    var loadAllPlaceholders = function (placeholderLoadArgs, callback)
    {
        ...
        masterSession.placeholderElement = document.documentElement; //no placeholder element as we have multiple root elements, so we just use the document to make the Html loader's event args constructor happy
    ...
    var setUpEventStream = function (session, callback)
    {
        ...
        session.eventStream.processReturnedEventArgs = function (htmlArgs, handlerResult, step)
        {
            ...
            {
                session.selectedChildPlaceholders = getSelectedChildPlaceholders(htmlArgs.childPlaceholders);
            }
        };
        ...
        {
            var completedLoad = new EVUI.Modules.HtmlLoader2.HtmlPlaceholderLoadResult();
            completedLoad.htmlPlaceholderLoadArgs = session.placeholderArgs;
            completedLoad.loadedChildResults = session.loadResults;
            completedLoad.placeholder = session.placeholderElement;
            completedLoad.placeholderID = session.placeholderID;
            completedLoad.loadedContent = session.loadedContent;


            if (typeof callback === "function")
            {
                return callback(completedLoad);
            }
        };
        ...
        eventStream.addStep(
            {
                ...
                handler: function (jobArgs)
                {
                    ...
                    for (var x = 0; x < numChildSessions; x++)
                    {
                        ...
                        session.childSessions.push(childSession);
    ...
    var PlaceholderLoadSession = function ()
    {
        ...
        /**The session that initiated this session.
        @type {PlaceholderLoadSession}*/
        this.parentSession = null;

        /**The arguments for loading the placeholder.
        ...
        /**String. The PlacehodlerID of the root placeholder being loaded.
        @type {String}*/

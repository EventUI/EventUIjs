...
/**Manager object for coordinating multiple global event handler contexts. Exists so that objects with global event handlers can be registered to have multiple global handlers that don't interfere with each other based on a context key.
@class*/
EVUI.Modules.EventStream.GlobalEventNamespaceManager = function (defaultEvents)
{
    ...
    this.addNamespace = function (namespace, events)
    {
        if (events == null || typeof events !== "object") throw Error("Object expected.");
        if (EVUI.Modules.Core.Utils.stringIsNullOrWhitespace(namespace) === true) throw Error("Context must have a non-whitespace string as its name.");
        if (_namespaces[namespace] != null) throw Error("A context with the name of \"" + contextName + "\" already exists.");

        var eventNamespace = new EVUI.Modules.EventStream.GlobalEventNamespace(namespace);
        eventNamespace.events = events;
        eventNamespace.remove = function ()
        {
            delete _namespaces[eventNamespace.namespace];
        };

        _namespaces[namespace] = eventNamespace;
        return eventNamespace;
    };

    /**Gets a event namespace based on its name.
    @param {String} namespace The name of the context to get. If a context with the given name does not exist, the default one is returned instead.
    @returns {EVUI.Modules.EventStream.GlobalEventNamespace}*/
    this.getNamespace = function (namespace)
    {
        if (namespace == null) return _defaultNamespace;

        var ns = (typeof namespace === "string") ? _namespaces[namespace] : null;
        return ns;
    };

    _defaultNamespace = new EVUI.Modules.EventStream.GlobalEventNamespace("bantmDefault");
    _defaultNamespace.events = (defaultEvents == null) ? {} : defaultEvents;
};

/**Object representing a context for "global" event handlers (handlers that are attached to a singleton instance of a controller) and their bubbling events.
@class*/
EVUI.Modules.EventStream.GlobalEventNamespace = function (namespace)
{
    if (typeof namespace !== "string") throw Error("String expected.");

    var _namespace = namespace;
    var _bubblingEvents = new EVUI.Modules.EventStream.BubblingEventManager(true);
    ...
    /**Object. The object from which the global events will be pulled from.
    @type {Object}*/
    this.events = {};

    /**Object. The manager for the bubbling events associated with this global context.
    @type {EVUI.Modules.EventStream.BubblingEventManager}*/
    this.bubblingEvents = null;
    Object.defineProperty(this, "bubblingEvents", {
        get: function ()
        {
            return _bubblingEvents;
        },
        configurable: false,
        enumerable: true
    });

    /**Removes the GlobalEventNamespace from its GlobalEventNamespaceManager.*/
